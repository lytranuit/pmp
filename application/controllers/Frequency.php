...
//        }
    ...
    public function add() { /////// trang ca nhan
        if (isset($_POST['dangtin'])) {
            ...
            $data_up = $this->frequency_model->create_object($data);
            $id = $this->frequency_model->insert($data_up);

            redirect('frequency', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
        } else {
            echo $this->blade->view()->make('page/page', $this->data)->render();
        }
    }
    ...
    public function edit($param) { /////// trang ca nhan
        ...
        if (isset($_POST['dangtin'])) {
            ...
            $data = $_POST;
            $data_up = $this->frequency_model->create_object($data);
            $this->frequency_model->update($data_up, $id);
            redirect('frequency', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
        } else {
            $this->load->model("frequency_model");
            $tin = $this->frequency_model->where(array('id' => $id))->as_object()->get();
            $this->data['tin'] = $tin;
...
//            load_chossen($this->data);
    ...
    public function remove($params) { /////// trang ca nhan
        $this->load->model("frequency_model");
        $id = $params[0];
        $this->frequency_model->update(array("deleted" => 1), $id);
...
//            $max_page = ceil($totalFiltered / $limit);
            ...
            $where = $this->frequency_model->where(array("deleted" => 0));
        } else {
            $search = $this->input->post('search')['value'];
            $sWhere = "deleted = 0 and (name like '%" . $search . "%')";
            $where = $this->frequency_model->where($sWhere, NULL, NULL, FALSE, FALSE, TRUE);
