...
class Factory extends MY_Controller
{
    ...
    { /////// trang ca nhan
        ...
        if (isset($_POST['dangtin'])) {
            ...
            $this->load->model("factory_model");
            $tin = $this->factory_model->where(array('id' => $id))->as_object()->get();
            $this->data['tin'] = $tin;
    ...
    public function table()
    {
        ...
        if (empty($this->input->post('search')['value'])) {
            ...
            $where = $this->factory_model->where(array("deleted" => 0));
        } else {
            $search = $this->input->post('search')['value'];
            $sWhere = "deleted = 0 and (name like '%" . $search . "%')";
            $where = $this->factory_model->where($sWhere, NULL, NULL, FALSE, FALSE, TRUE);
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                ...
                $nestedData['action'] = '<a href="' . base_url() . 'factory/edit/' . $post->id . '" class="btn btn-warning btn-sm mr-2" title="edit">'
                    ...
                    . '</i>'
                    . '</a>'
                    . '<a href="' . base_url() . 'factory/remove/' . $post->id . '" class="btn btn-danger btn-sm" data-type="confirm" title="remove">'
