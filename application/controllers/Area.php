...
class Area extends MY_Controller
{
    ...
    { /////// trang ca nhan
        ...
        if (isset($_POST['dangtin'])) {
            ...
            $this->load->model("area_model");
            $tin = $this->area_model->where(array('id' => $id))->as_object()->get();
            $this->data['tin'] = $tin;
    ...
    public function table()
    {
        ...
        if (empty($this->input->post('search')['value'])) {
            ...
            $where = $this->area_model->where($sWhere, NULL, NULL, FALSE, FALSE, TRUE);
            $totalFiltered = $where->count_rows();
            $where = $this->area_model->where($sWhere, NULL, NULL, FALSE, FALSE, TRUE);
        }

        $posts = $where->order_by("id", "DESC")->with_factory()->with_workshop()->paginate($limit, NULL, $page);
        //        echo "<pre>";
        //        print_r($posts);
        //        die();
        $data = array();
